...
router.post("/create-payment-intent", authenticate, asyncHandler(async (req, res) => {
  const { listingId, amount, currency = "inr" } = req.body;
  ...
  if (!listingId || !normalizedAmount || amountInSmallestUnit <= 0) {
...
router.post("/webhook", express.raw({ type: "application/json" }), asyncHandler(async (req, res) => {
  ...
  if (!stripe || !signature || !env.stripe.webhookSecret) {
    return res.status(400).json({ message: "Missing webhook signature configuration" });
  }
